...
include_once'Resource/utilities.php';
  ...
  if(isset($_POST['signupButton']))
  {
    ...
    if(checkDuplication("artist","email",$email,$db))
    {
      $result = flashMessage("Email is already taken, Please try another one.");
    }
    else if(checkDuplication("artist","username",$username,$db))
    {
    ...
    else if(empty($form_errors))
    {
      ...
      try
      {
        $sqlInsert = "INSERT INTO artist (username, email, password, first_name, last_name, phone, join_date)
                      VALUES (:username, :email, :password, :first_name, :last_name, :phone, now())";
        $statement = $db->prepare($sqlInsert);
        $statement->execute(array(':username' =>$username, ':email'=> $email, ':password'=>$hashed_password,':first_name'=>$firstname,':last_name'=>$lastname,':phone'=>$phonenumber));
